...
ts_wconf <- ts %>% right_join(county_pm25, by = c("StateCounty_FIPS" = "fips" , "year" = "year")) %>%
  right_join(temp_seasonal_county, by = c("StateCounty_FIPS" = "fips" , "year" = "year")) %>%
  right_join(brfss_county_interpolated, by = c("StateCounty_FIPS" = "fips" , "year" = "year")) %>%
  right_join(census_county_interpolated, by = c("StateCounty_FIPS" = "fips" , "year" = "year")) %>%
